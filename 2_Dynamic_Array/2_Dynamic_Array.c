...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
    // Array of pointers to hold the dynamic arrays
    int** arr = (int**)calloc(n, sizeof(int*));
    int* arr_sizes = (int*)calloc(n, sizeof(int));
    int* arr_capacities = (int*)calloc(n, sizeof(int));
    
    // Array to store the answers for type 2 queries
    ...
    for (int i = 0; i < queries_rows; i++) {
        ...
        if (q_type == 1) {
            // Resize the specific array if capacity is reached
            if (arr_sizes[idx] == arr_capacities[idx]) {
                arr_capacities[idx] = (arr_capacities[idx] == 0) ? 1 : arr_capacities[idx] * 2;
                arr[idx] = (int*)realloc(arr[idx], arr_capacities[idx] * sizeof(int));
            }
            arr[idx][arr_sizes[idx]] = y;
            arr_sizes[idx]++;
        } else if (q_type == 2) {
            // Find the target value and append to answers
            int inner_idx = y % arr_sizes[idx];
            lastAnswer = arr[idx][inner_idx];
            answers[answer_idx++] = lastAnswer;
        }
    }
    
    // Free the inner dynamic arrays and tracking arrays to prevent memory leaks
    for (int i = 0; i < n; i++) {
        if (arr[i] != NULL) {
            free(arr[i]);
        }
